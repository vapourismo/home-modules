...
vim.api.nvim_create_user_command("PickJJ", function()
	local jj_proc = io.popen(
		[[jj log -r 'summary()' --no-graph -T 'change_id.shortest() ++ ":\t" ++ if(bookmarks, "[" ++ bookmarks ++ "]\t", "") ++ if(description, description.first_line(), label(if(empty, "empty"), description_placeholder)) ++ "\n"']]
	)
