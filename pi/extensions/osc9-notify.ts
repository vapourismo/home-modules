...
export function formatOsc9Notification(message: string): string {
	...
	const title = message.replace(/[\x00-\x1f\x7f-\x9f]/g, " ").trim() || "Pi needs your attention";
...
export function sendOsc9Notification(
	message: string,
	write: (sequence: string) => unknown = (sequence) => process.stdout.write(sequence),
): void {
	write(formatOsc9Notification(message));
}

function isApprovalPrompt(title: string): boolean {
	return /\b(?:allow|approval|approve|authorize|blocked|permission)\b/i.test(title);
}
...
export default function osc9Notify(pi: ExtensionAPI): void {
	...
	const notify = (message: string): void => {
		...
		const now = Date.now();
		if (message === lastMessage && now - lastNotificationAt < REPEAT_DEBOUNCE_MS) return;
	...
	const hookAgentDialogs = (ctx: ExtensionContext): void => {
		...
		const maybeNotify = (title: string): void => {
			...
			notify(isApprovalPrompt(title) ? "Pi: approval needed" : "Pi: input needed");
		};

		const wrappedSelect: ExtensionUIContext["select"] = (title, options, opts) => {
			maybeNotify(title);
			return originalSelect.call(ui, title, options, opts);
		};
		const wrappedConfirm: ExtensionUIContext["confirm"] = (title, message, opts) => {
			maybeNotify(title);
			return originalConfirm.call(ui, title, message, opts);
		};
		const wrappedInput: ExtensionUIContext["input"] = (title, placeholder, opts) => {
			maybeNotify(title);
